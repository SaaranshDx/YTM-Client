...
function ensureDefaultSettings() {
    return new Promise((resolve) => {
        STORAGE.get(DEFAULT_SETTINGS, function (result) {
            STORAGE.set({
                isYtOn: result.isYtOn !== undefined ? result.isYtOn : true,
                isYtmOn: result.isYtmOn !== undefined ? result.isYtmOn : true,
                debugMode: result.debugMode !== undefined ? result.debugMode : false,
...
function getSubscribedChannels() {
    return new Promise((resolve, _) => {
        STORAGE.get({ subscribes: {} }, function (result) {
...
function checkIfServiceEnabled(serviceName) {
    ...
    return new Promise((resolve, _) => {
        ...
        STORAGE.get(serviceName, function (result) {
            var isEnabled = true;
...
function checkIfDebugModeEnabled() {
    ...
    return new Promise((resolve, _) => {
        STORAGE.get({ debugMode: false }, function (result) {
...
function handleSubscriptionUpdate(request, sender, sendResponse) {
    ...
    getSubscribedChannels().then(subscribes => {
        ...
        STORAGE.set({ subscribes: subscribes }, function () {
...
function handleServiceStatusUpdate(request, sender, sendResponse) {
    ...
    STORAGE.set({ [serviceName]: isEnabled }, function () {
        sendResponse({ success: true });
    });
...
function handleDebugModeUpdate(request, sender, sendResponse) {
    ...
    STORAGE.set({ debugMode: isDebugModeEnabled }, function () {
        sendResponse({ success: true });
        broadcastStatusUpdate(
            'debug-mode-updated', {
